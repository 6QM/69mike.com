...
网站会在 Hugo 构建阶段把 LaTeX 公式转换为 KaTeX HTML 和 MathML，不需要在文章 front matter 中另外开启数学渲染。

行内公式使用 `\(...\)`：

```latex
这一轮的新增投入是 \(I_t\)，已有资产存量是 \(K_t\)。
```
...
块级公式也支持 `$$...$$`。不要使用单个美元符号 `$...$` 书写行内公式：网站没有启用这种分隔方式，以免文章中的货币符号被误判。公式过宽时会在公式区域内横向滚动，不应撑宽整篇文章。
